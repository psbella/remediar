#!/usr/bin/env python3
"""
normalizar.py — Aplica las 8 capas de corrección sobre raw.json.

Input:  data/raw.json         (generado por extraer.py)
Output: data/normalizado.json (mismo esquema + campos corregidos)

Capas:
  0  reparar_droga_faltante    — droga/marca desplazadas por omisión en PDF
  2  rescatar_laboratorios     — lab en slot de presentacion
  3  reparar_denver            — fusiones de Denver Farma
  4  reparar_marca_desplazada  — nombre comercial en campo droga
  5  extraer_presentacion_de_marca + reparar_presentacion_desplazada + limpiar_dosis_residual_en_marca
  6  crosswalk_pami            — recupera droga, corrige lab, agrega pami_cobertura
  7  aplicar_droga_fixes       — correcciones manuales desde droga_fixes.json
  +  blacklist, outliers, vigencia_score
"""
...
def main():
    if not RAW_PATH.exists():
        print(f"ERROR: {RAW_PATH} no encontrado. Ejecutá primero extraer.py")
        sys.exit(1)
    ...
    print(f"Registros crudos: {len(medicamentos)}")
    ...
    medicamentos, n = reparar_droga_faltante(medicamentos, fixes_droga)
    print(f"   Reparados: {n}")

    # ── CAPA 2 ────────────────────────────────────────────────────────────
    print("\nRescatando laboratorios desplazados...")
    medicamentos, n = rescatar_laboratorios(medicamentos)
    n_desc = sum(1 for m in medicamentos if m.get("laboratorio") == "Desconocido")
    print(f"   Rescatados: {n} | Sin recuperar: {n_desc}")

    # ── CAPA 3 ────────────────────────────────────────────────────────────
    print("\nReparando registros Denver Farma...")
    medicamentos, n = reparar_denver(medicamentos)
    print(f"   Reparados: {n}")

    # ── CAPA 4 ────────────────────────────────────────────────────────────
    print("\nReparando marcas desplazadas...")
    medicamentos, n = reparar_marca_desplazada(medicamentos)
    print(f"   Reparados: {n}")

    # ── CAPA 5 ────────────────────────────────────────────────────────────
    print("\nExtrayendo presentacion de marca fusionada...")
    medicamentos, n = extraer_presentacion_de_marca(medicamentos)
    print(f"   Reparados: {n}")

    print("\nReparando presentacion desplazada...")
    medicamentos, n = reparar_presentacion_desplazada(medicamentos)
    print(f"   Reparados: {n}")

    print("\nLimpiando dosis residual en marca...")
    medicamentos, n = limpiar_dosis_residual_en_marca(medicamentos)
    print(f"   Reparados: {n}")

    # ── CAPA 6 ────────────────────────────────────────────────────────────
    print("\nCrosswalk PAMI...")
    medicamentos, stats = crosswalk_pami(medicamentos)
    print(f"   Matches exactos: {stats['match_exacto']} | "
          f"Drogas recuperadas: {stats['droga_recuperada']} | "
          f"Labs corregidos: {stats['lab_corregido']}")

    # ── CAPA 7 ────────────────────────────────────────────────────────────
    print("\nAplicando fixes manuales de droga...")
    medicamentos, n = aplicar_droga_fixes(medicamentos)
    print(f"   Corregidos: {n}")

    # ── BLACKLIST + OUTLIERS + VIGENCIA ───────────────────────────────────
    ...
    NORMALIZADO_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(NORMALIZADO_PATH, "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, separators=(",", ":"))

    print(f"\nGuardado: {NORMALIZADO_PATH}")
    print(f"Total: {len(medicamentos)} | Excluidos (blacklist): {n_bl}")
